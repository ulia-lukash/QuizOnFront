    ...
    <div class="form center">
      ...
      <form class="btn_container center">
        ...
        <div class="agreement">
            ...
            <a  href="https://bmstu.ru/about/obrabotka-dannyh" class="agreement_link"> обработку персональных данных</a>
        </div>
    ...
    async checkIfCanRegister() {
      try {
        ...
        if (available === 'reserve') {
          this.headerText = 'Упс... 👉🏻👈🏻'
          this.bodyText =
            'К сожалению, места на&nbspигру закончились. Но не стоит опускать руки раньше времени! Мы можем зарегистрировать твою команду в&nbspрезерв, чтобы в&nbspслучае отказа  от&nbspкакой-то из прошедших команд, вы могли занять их место. Хочешь зарегистрироваться в&nbspрезерв?'
          this.showModal()
        }
        return available
      } catch (error) {
        ...
        this.postData('https://www.quiz-on.ru/api/register', this.form).then(
          (response) => {
            if (response.status !== 200) {
              ...
              this.headerText =
                '🤓 Поздравляем, первые задания  от&nbspКвизON успешно выполнены — твоя команда зарегистрирована!'

              this.bodyText =
                'Посмотрим, как ты справишься с другими вопросами на&nbspвторой игре Бауманской лиги КвизON. Напомним, что игра пройдет:'
              this.gameDate = '⚡️ 24 октября, 19:00'
              this.gameLoc = '⚡️ 345 ауд. (ГУК)'
              this.seeYa = ' до&nbspвстречи на&nbspигре!'
              this.showModal()
              this.form = {
                tg_contact: '',
                team_id: null,
