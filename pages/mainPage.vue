...
{{ headerText}}  
  ...
  <template v-slot:footer>
    ...
    <div v-on:click="checkAvailability" class="play-button">
      <nuxt-link :event="disabled ? '' : 'click'"  class="link" to="/RegisterPage">
    ...
    <div class="frame second">
      ...
      <div class="text-group">
        ...
        <p>это интеллектуально-развлекательная игра в формате викторины, которая станет отличным вариантом для отдыха в хорошей компании</p>
        <h2 class="hcolor">ЧТО НУЖНО СДЕЛАТЬ?</h2>
        <p>Собрать команду от 4 до 8 человек, зарегистрировать ее на сайте и прийти в указанные время и место на игру</p>
        <h2 class="hcolor">КАК ЭТО ПРОИСХОДИТ?</h2>
        <p>Игра включает в себя несколько раундов и самые разные форматы вопросов. Вместе с командой вам предстоит побороться в этой битве логики, эрудиции и скорости мышления!</p>
      </div>
    </div>
    <div class="frame third">
      ...
      <p style="margin-top: 2vh"> <span style="font-weight: 700; font-size: 20px;">Вопросы на различные темы:</span> от классической музыки до современных сериалов и кино, от науки до бытовой культуры</p>
      <p style="margin-top: 2vh;"><span style="font-weight: 700; font-size: 20px;">2 часа</span> интеллектуального сражения за звание самых умных игроков вечера и целого сезона</p>
    </div>
  </div>
</template>
  ...
  components: { ModalVue },
    ...
    methods: {
      ...
      async checkIfCanRegister() {
        try {
          const response = await fetch("https://www.quiz-on.ru/api/register-available").then(respone => {
            return respone.json()
          })
...
.third {
  background-image: radial-gradient(60% 90% at 40.78% 65.16%, rgba(0, 0, 0, 0.529) 0%, rgba(0, 0, 0, 0.648) 25.74%, rgba(15, 26, 46, 0.64) 53.56%, #182A3E 100%), url("../assets/images/background-third.jpg");
